...
struct ELEVFILEHEADER { // file header for patch elevation data file
    ...
    int dtype;             // data format (0=flat, no data block; 8=uint8; -8=int8; 16=uint16_t; -16=int16_t)
...
ElevData elvscan(const BYTE *data, int ndata)
{
	...
	case -16: {
        int16_t *data_int16_t = (int16_t*)data;
		for (i = 0; i < ndat; i++)
            e[i] = (double)data_int16_t[i] * scale + offset;
		} break;
	}
