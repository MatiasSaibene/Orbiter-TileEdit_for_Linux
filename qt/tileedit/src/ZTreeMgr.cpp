...
{
    uLongf ndata = static_cast<uLongf>(noutp); // Convertimos noutp a uLongf
    if (uncompress(outp, &ndata, inp, static_cast<uLong>(ninp)) != Z_OK)
        return 0;
    return static_cast<DWORD>(ndata); // Convertimos ndata de nuevo a DWORD
}
