    ...
    , ui(new Ui::tileedit)
{
    ...
    strncpy(path, exePath.c_str(), 1024);

    // Usar std::filesystem para descomponer la ruta en Linux
    std::filesystem::path exePathObj(path);
    std::string dirStr = exePathObj.parent_path().string();
    std::string nameStr = exePathObj.stem().string();
    std::string extStr = exePathObj.extension().string();

    strncpy(drive, "", sizeof(drive));  // No se usa en Linux, así que lo dejamos vacío
    strncpy(dir, dirStr.c_str(), sizeof(dir));
    strncpy(name, nameStr.c_str(), sizeof(name));
    strncpy(ext, extStr.c_str(), sizeof(ext));
#endif

    // Formatear el path para el archivo .ini
    if (!dirStr.empty() && dirStr.back() != '/') {
        dirStr += '/';
    }

    // Construir la ruta correctamente
    snprintf(path, sizeof(path), "%s%s%s.ini", drive, dirStr.c_str(), nameStr.c_str());
    ...
    m_sTileBlock = nullptr;
    m_mTileBlock = nullptr;
    m_eTileBlock = nullptr;
    m_eTileBlockRef = nullptr;

    m_mgrSurf = nullptr;
    m_mgrMask = nullptr;
    m_mgrElev = nullptr;
    m_mgrElevMod = nullptr;

    m_dlgElevConfig = nullptr;
    ...
    connect(ui->bgrpAction, &QButtonGroup::idClicked, this, &::tileedit::onActionButtonClicked);

    ui->bgrpAction->addButton(ui->btnActionNavigate, 0);
    ui->bgrpAction->addButton(ui->btnActionElevEdit, 1);

	ui->bgrpEdit = new QButtonGroup(this);  // Inicializa el grupo de botones
	ui->bgrpEdit->addButton(ui->btnElevPaint, 0);
	ui->bgrpEdit->addButton(ui->btnElevRandom, 1);
	ui->bgrpEdit->addButton(ui->btnElevErase, 2);

    connect(ui->bgrpEdit, &QButtonGroup::idClicked, this, &::tileedit::onActionButtonClicked);
...
void tileedit::onActionButtonClicked(int id) {
    ...
    QAbstractButton* button = ui->bgrpAction->button(id);
